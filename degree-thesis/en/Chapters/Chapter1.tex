...
under the \enquote{Additional Documentation} section towards the bottom of the page.\sidenote{
...
Another option is typesetting the template with XeTeX. That works once you have removed the line \verb|\usepackage[utf8]{luainputenc}| from \file{setup.tex} as well as all calls to \code{textls}.


\section{Getting Started with the Template}

Once you are familiar with LaTeX, you should explore the directory structure of the template. After that, place your details into the \emph{THESIS INFORMATION} block of the \file{main.tex} file. You can then modify the rest of this file to your unique specifications based on your degree/university. \Cref{FillingFile} on page~\pageref{FillingFile} will help you do this. Make sure you also read \Cref{ThesisFeatures} about the template features.
...
\keyword{setup.tex} -- This file loads and sets up additional LaTeX packages. It changes some defaults of the MastersDoctoralThesis, on which this template is based, and controls the layout of the thesis.

Files that are \emph{not} included, but are created by LaTeX as auxiliary files include: \file{main.aux}, \file{main.bbl}, \file{main.aux}, \file{main.blg}, 
...
You have to compile this template with \texttt{LuaLaTeX} (cf. Sect.~\ref{sec:requirements}). Using \texttt{pdLaTeX}, however, is not possible, because the template uses TTF and OTF fonts.
...
On Linux and macOS, you can use the provided \keyword{Makefile}. Just navigate to the ``en'' directory and enter \code{make} in a terminal. Running \code{make} will automatically call the programs \code{lualatex} (which creates the PDF) and \code{biber} (which is used to compile the bibliography).
...
Alternatively, you should be able to compile the thesis by executing \code{latexmk -lualatex -pdf main.tex}.
...
The next block sets up the bibliography. The template uses the bibliography style \emph{alpha}. The alpha style creates reference labels that contain the first letters or initials of authors and a two-digit number for the year.
