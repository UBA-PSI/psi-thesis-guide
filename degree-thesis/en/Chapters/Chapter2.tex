...
Content may change or move to different URLs, and sites may go out of service altogether. This phenomenon is called Link Rot, and it has become a significant issue in academia.\sidenote{See also: \href{https://en.wikipedia.org/wiki/Link_rot}{Link Rot in Wikipedia}}
...
\sidenote{We have archived all websites mentioned in this guide. In the interest of readability, we will omit the suffix after links.}
...
As shown in \Cref{tab:treatments}, \textbf{column heads} should use sentence-style capitalization, i.\,e., do not use \emph{title case} as you would in section headings.
...
Watch the illustrative presentation entitled ``Remove to improve.''\marginnote{\url{https://www.darkhorseanalytics.com/blog/data-looks-better-naked/}} The plots in Fig.~\ref{fig:barplot} (taken from linked website) demonstrate the benefits of removing unnecessary clutter.
...
You can also use the following custom command \code{wideimage} for inserting a figure.
...
Often, it makes sense to define custom hyphenation rules globally. To this end, define your custom hyphenation definitions with\todo[noline]{Oh no, an overful hbox!}\ \verb|\hyphenation{FORTRAN Hy-phen-a-tion}| and insert them after the \verb|\begin{document}| clause.
