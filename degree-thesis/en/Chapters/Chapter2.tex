...
Example of more complex tables are Tables~\ref{tab:complex} and \ref{tab:complextext}. Table~\ref{tab:complextext} shows how to build wide tables that span the margin and the text. If you need to create similar tables, feel free to re-use the code in \file{Chapter2.tex}.
...
Ideally, you draw all figures yourself to ensure that they use a \textbf{consistent graphical language}, which includes colors,  fonts, text sizes, line thickness, arrowheads, fill styles, padding, margins, and the radius of rounded corners.
...
If you present plots of experimental results, use a tool that allows you to \textbf{recreate the plots} without any effort. Write scripts that create the plots based on raw data, so you can recreate the plots easily. You will also be able to change the design of all plots with little effort. Common tools are Python's \textsc{matplotlib} (also consider using \textsc{seaborn}), R's \textsc{ggplot2}, and \textsc{gnuplot}. Make sure to export your plots as PDFs.
