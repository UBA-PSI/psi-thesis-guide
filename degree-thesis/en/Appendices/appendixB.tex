...
In this section we review common design principles for graphs (also called charts or plots). To this end, we present the examples shown by Carter \cite{Carter12}.

A general advice for any graph is to have legible axis labels and, if more than one data series is visualized, a legend for the data series. Moreover, remember to use colors effectively (cf. Sect.~\ref{sec:color}).
...
Often, there are many lines to be plotted. Resist the temptation to add more than three lines into one line chart if the lines overlap. Instead, create multiple charts where one of the lines is repeated to serve as a baseline.
...
\sidecaption{\label{fig:linecharts} Advice for line charts \cite{Carter12}. Remark on the layout: As seen in this case, putting a wide figure below another one does not look very pleasing.}[-1\baselineskip]
\end{figure*}
...
The bars can be drawn horizontally and vertically. The advice in this section assumes vertical bars. In vertical bar charts the Y-axis corresponds to the value of the variable and the X-axis is typically used for a categorial variable, which represents the different circumstances.

Figure~\ref{fig:barcharts} summarizes Carter's advice on bar charts.

\begin{figure*}[t]
\centering
\vspace{4\baselineskip}
\includegraphics[width=1\widefigurewidth]{graphs-barcharts}
\sidecaption{\label{fig:barcharts} Advice for bar charts \cite{Carter12}.}
\end{figure*}

\paragraph{Histograms} A special kind of bar graphs are histograms. Carter gives a concise explanation of the purpose of a histogram: ``A histogram shows the distribution of data and the relative frequency with which the data occur. It essentially offers the audience an estimate of the probability distribution of a dataset'' \cite{Carter12}.

Figure~\ref{fig:histograms} summarizes Carter's advice on histograms.
