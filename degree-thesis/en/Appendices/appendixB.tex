...
Line charts show how one variable (the one on the y-axis) changes when another one is varied within a given range. Line charts are particularly useful to visualize time series. Figure~\ref{fig:linecharts} summarizes Carter's advice on line charts.

\begin{figure*}[t]
\centering
\includegraphics[width=1\widefigurewidth]{graphs-linecharts}
\sidecaption{\label{fig:linecharts} Advice for line charts \cite{Carter12}.}[-1\baselineskip]
\end{figure*}
