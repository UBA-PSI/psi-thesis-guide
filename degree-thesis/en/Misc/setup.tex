...
% alignment in all other cases; we might decide to remove this again
...
% make listings floating
\usepackage{float}

% Define our own float style "listing" based on "plaintop"
% used to reduce vspace between caption and lstlisting
% which otherwise would be artificialy large due to
% lstlisting's aboveskip line (which we want to keep for
% the case that a listing is *not* embedded in a float)
\makeatletter
\newcommand\fs@listing{\fs@plain
\def\@fs@mid{\vspace\belowcaptionskip\relax}%
\let\@fs@iftopcapt\iftrue}
\let\floatc@plaintop=\floatc@plain
\makeatother

% defining the lstfloat environment
\floatstyle{listing}
